...
LAB_MARKER="DOCKER-LAB-DAY1"   # Day1 실습에서 페이지에 넣어야 하는 표식
...
lab_init() {
  ...
  if ! docker info >/dev/null 2>&1; then
    echo -e "${C_R}[오류]${C_N} Docker 데몬에 연결하지 못했습니다."
    echo "        newgrp docker 를 실행하거나 세션을 다시 여십시오."
    exit 1
  fi
  echo ""
  echo -e "${C_B}[${LAB_ID}] ${LAB_NAME}${C_N}"
  echo -e "  호스트: $(hostname)   시각: $(date '+%Y-%m-%d %H:%M:%S')"
  echo "  ------------------------------------------------------------"
}
...
lab_finish() {
  ...
  echo "  결과 파일: ${out}"
  if [ "$pct" -eq 100 ]; then
    echo -e "  ${C_G}전 항목을 통과했습니다. 이 화면을 강사에게 보여주십시오.${C_N}"
  else
    echo "  FAIL 항목을 고친 뒤 이 스크립트를 다시 실행하십시오. 횟수 제한은 없습니다."
  fi
  echo ""
}
